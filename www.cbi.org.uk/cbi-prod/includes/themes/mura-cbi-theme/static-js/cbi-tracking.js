...
if (navigator.doNotTrack == "yes" || navigator.doNotTrack == "1" || navigator.msDoNotTrack == "1") { 
	...
	if (document.cookie.indexOf("optout") > -1) {} else {
		$("body").prepend(
			"<div id='cookies'>" +
			"	<div class='inner'>" + 
			"		<p class='left'>This website uses cookies to improve user experience." + 
			"			<a href='/about/privacy-policy/' title='Find out more about the cookies used on this website'>Find out more</a>" +
			"		</p>" +
			"		<p class='right'>" +
			"			<span class='in'>Opt-in</span>" +
			"			<span class='out'>Opt-out</span>" +
			"		</p>" +
			"	</div>" +
			"</div>"
		);
		$("#cookies").delay(750).slideDown();
		$("#cookies .in").click(function(){ optin(); });
		$("#cookies .out").click(function(){ optout(); });
	}
}
$("section .main span#optin").click(function(){ optin(); });
	...
	if (document.cookie.indexOf("optout") >= 0) {} else {
		...
		if (window.location.href.indexOf("businessvoice") > -1) {
			$.getScript("//platform.twitter.com/oct.js", function() {
				twttr.conversion.trackPid('nuwk5', { tw_sale_amount: 0, tw_order_quantity: 0 });
			});
		}
	}
}
